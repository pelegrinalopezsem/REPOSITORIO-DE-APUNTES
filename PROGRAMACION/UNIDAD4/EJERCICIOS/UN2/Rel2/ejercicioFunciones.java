package PROGRAMACION.UNIDAD_4.EJERCICIOS.UN2.Rel2;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Scanner;

public class ejercicioFunciones {
    // FUNCION PARA COMPROBAR SI EL STRING NO ESTA REPETIDO EN EL ARRAY
    public static void aniadirSinDuplicados(String mensajeString, ArrayList<String> array, Scanner sc) {
    boolean duplicado;
    String posibleDuplicado;

    do {
        duplicado = false;
        imprimirMensaje(mensajeString);
        posibleDuplicado = sc.nextLine().trim();

        for (String item : array) {
            if (item.equalsIgnoreCase(posibleDuplicado)) {
                duplicado = true;
                break;
            }
        }

        if (duplicado) {
            imprimirMensaje("No se añade \"" + posibleDuplicado + "\" ya que está en el array. Intenta otra vez.");
        } else {
            array.add(posibleDuplicado);
        }

    } while (duplicado);
}


    // LEE EL STRING Y EL ARRAY QUE QUIERAS INTRODUCIR
    public static void leerStringArray(String mensajeString, ArrayList<String> array, Scanner sc) {
        String stringUsuario;
        imprimirMensaje(mensajeString);
        stringUsuario = sc.nextLine();
        aniadirSinDuplicados(mensajeString, array, sc);

    }

    // PARA IMPRIMIR EN CONSOLA EL MENSAJE QUE QUIERAS
    public static void imprimirMensaje(String mensajeAMostrar) {
        System.out.println(mensajeAMostrar);
    }

    // LEE EL INT PARA INTRODUCIRLO EN EL ARRAY
    public static void leerIntArray(String mensaString, ArrayList<Integer> array, Scanner scanner) {
        int numeroUsuario;
        imprimirMensaje(mensaString);
        numeroUsuario = scanner.nextInt();
    }

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        ArrayList<String> libros = new ArrayList<>(
                Arrays.asList("El quijote", "La Biblia", "Manual de resistencia", "La criada"));
        ArrayList<Integer> cantidades = new ArrayList<>(Arrays.asList(5, 3, 1, 7));

        String opcion = "";
        String libroNuevo;
        int cantidadLibroNuevo;
        String libroActualizado;
        int cantidadLibroActualizado;
        boolean encontrado;
        String mensaje;

        while (!opcion.equals("4")) {
            System.out.println("1. Añadir un nuevo libro\n" +
                    "2. Actualizar cantidad\n" +
                    "3. Ver inventario\n" +
                    "4. Salir");

            opcion = scanner.nextLine();

            if (opcion.equals("1")) {

                leerStringArray("Introduzca el nombre del nuevo libro", libros, scanner);
                leerIntArray("Cantidad disponible: ", cantidades, scanner);

            } else if (opcion.equals("2")) {
                System.out.println("Nombre del libro a Actualizar");
                libroActualizado = scanner.nextLine();

                System.out.println("Nueva cantidad: ");
                cantidadLibroActualizado = scanner.nextInt();
                scanner.nextLine();

                encontrado = false;

                for (int i = 0; i < libros.size() && !encontrado; i++) {
                    if (libros.get(i).equalsIgnoreCase(libroActualizado)) {
                        encontrado = true;
                        cantidades.set(i, cantidadLibroActualizado);
                    }
                }

                if (!encontrado) {
                    System.out.println("Libro no encontrado");
                }

            } else if (opcion.equals("3")) {
                System.out.println("Inventario: ");
                mensaje = "";
                for (int i = 0; i < libros.size(); i++) {
                    mensaje += libros.get(i) + " - " + cantidades.get(i) + " unidades\n";
                }

                System.out.println(mensaje);

            } else if (opcion.equals("4")) {
                System.out.println("Programa finalizado");
            } else {
                System.out.println("Introduce una opción válida");
            }
        }

        scanner.close();
    }
}
