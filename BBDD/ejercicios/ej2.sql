...
SELECT nombre_jugador FROM jugadores WHERE id_equipo = 1 OR id_equipo = 2 OR id_equipo = 3;
...
SELECT * FROM equipos ORDER BY nombre_equipo ASC;
...
SELECT id_equipo, AVG(goles) AS goles_promedio
FROM jugadores
GROUP BY id_equipo
HAVING AVG(id_equipo) = 1;
-- 9. Mostrar partidos jugados en enero de 2023.
SELECT * FROM partidos WHERE fecha >= 2023-01-01 AND fecha <= 2023-12-31;
-- 10. Contar jugadores de un equipo con ID específico.

SELECT id_equipo, AVG(id_equipo)
FROM jugadores
GROUP BY id_equipo
HAVING AVG(id_equipo) = 1;
-- 11. Ordenar equipos por ciudad alfabéticamente y despues por año de fundacion descendente.
SELECT * FROM equipos ORDER BY nombre_equipo ASC, fundado DESC;
...
SELECT nombre_jugador ,AVG(goles) AS promedio_goles, AVG(asistencias) AS promedio_asistencias
FROM jugadores
...
SELECT id_equipo_local , COUNT(goles_local) AS total_goles
FROM partidos
